...
% prompt for tc track if not given
if isempty(tc_track)
...
% B = [0.7 0.4 0.4];
% C = [100 100 100]; 
wind_weakening(:,1) = A(1)*exp(-centroids.dist_to_coast / C(1));
wind_weakening(:,2) = A(2)*exp(-centroids.dist_to_coast / C(2));
wind_weakening(:,3) = A(3)*exp(-centroids.dist_to_coast / C(3));
wind_weakening(wind_weakening(:,1) < B(1), 1) = B(1);
wind_weakening(wind_weakening(:,2) < B(2), 2) = B(2);
wind_weakening(wind_weakening(:,3) < B(3), 3) = B(3);
...
if check_figure
    ...
    legendstr = {'Tropical depression and storm','Hurricane Cat. 1 and 2','Hurricane Cat. 3, 4 and 5'};
    for t_i = 1:size(A,2)
        legendstr{t_i} = sprintf('%s, y = %2.2f * e^{-1/%3.0f * distance}',legendstr{t_i}, A(t_i), C(t_i));
    end
    legend(h,legendstr,'location','southwest')
    legend('boxoff')
    ylim([0 1.05]) %ylim([0 max(wind_weakening(:))*1.05])
    xlim([0 200])
end
...
%Tropical storm
cat_0  = tc_cat <= 0;
hazard.intensity(cat_0'  ,:)= bsxfun(@times,hazard.intensity(cat_0'  ,:),wind_weakening(:,1)');

% Hurricane Cat. 1 2
cat_12  = tc_cat >0 & tc_cat <3;
hazard.intensity(cat_12' ,:)= bsxfun(@times,hazard.intensity(cat_12' ,:),wind_weakening(:,2)');

% Hurricane Cat. 3 4 5
cat_345 = tc_cat >= 3;
hazard.intensity(cat_345',:)= bsxfun(@times,hazard.intensity(cat_345',:),wind_weakening(:,3)');


% [i j x] = find(hazard.intensity);
...
% %Tropical storm
% cat_0  = tc_cat <= 0;
% index_ = ismember(j, find(cat_0));
% % unique(j(index_))
% x(index_) = x(index_) .* wind_weakening(:,1); 
...
% sea_index  = centroids.onLand == 0;
% hazard.intensity_w(:,sea_index) = hazard.intensity(:,sea_index);
