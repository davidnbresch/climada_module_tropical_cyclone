...
%Tropical storm
cat_0  = tc_cat <= 0;
hazard.arr(cat_0'  ,:)= bsxfun(@times,hazard.arr(cat_0'  ,:),wind_weakening(:,1)');

% Hurricane Cat. 1 2
cat_12  = tc_cat >0 & tc_cat <3;
hazard.arr(cat_12' ,:)= bsxfun(@times,hazard.arr(cat_12' ,:),wind_weakening(:,2)');

% Hurricane Cat. 3 4 5
cat_345 = tc_cat >= 3;
hazard.arr(cat_345',:)= bsxfun(@times,hazard.arr(cat_345',:),wind_weakening(:,3)');


% [i j x] = find(hazard.arr);
...
% hazard.arr_w(:,sea_index) = hazard.arr(:,sea_index);
