function [res,tc_track] = climada_tc_windfield_fast(tc_track,centroids,~,silent_mode,~)
...
%
% CALLING SEQUENCE:
%   [res,tc_track]=climada_tc_windfield_fast(tc_track,centroids,~,silent_mode,~)
% EXAMPLE:
%   tc_track=climada_tc_track_load('TEST_tracks.atl_hist');
%   centroids=climada_centroids_load('USFL_MiamiDadeBrowardPalmBeach');
%   res=climada_tc_windfield_fast(tc_track(68),centroids);
...
%   two DUMMY parameters kept (above shown as ~), i.e.
%   equal_timestep: disabled, just kept for backward compatibility
%   check_plot: disabled, just kept for backward compatibility when
%       replacing this code with older calls
%       for a simple check plot, use climada_color_plot(res.gust,res.lon,res.lat)
% OUTPUTS:
...
% David N. Bresch, david.bresch@gmail.com, 20160528, about 20% faster than climada_tc_windfield
%-
...
%parfor centroid_i=1:n_valid_centroids % works with PARFOR, but not faster
    ...
    if D<5*R % focus on the radius that really has an impact
        ...
        local_gust(centroid_i) = max((S/3.6)*1.27,0); % G now in m/s, peak gust
